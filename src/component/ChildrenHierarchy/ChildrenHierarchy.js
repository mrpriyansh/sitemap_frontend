...
function ChildrenHierarchy({ data, parentTitle, parentUrl }) {
  ...
  const comparator = (a, b) => {
    ...
    if (
      !response.mapping[b.url] ||
      response.mapping[b.url]?.children?.length < response.mapping[a.url]?.children?.length
    )
      return -1;
    return 1;
  };
            ...
            {data.map((child, i) => {
              return (
                <Page
                  ...
                  url={child.url}
                  isParent={false}
                  isPapa={!i && response.mapping[data[0]?.url]?.children?.length}
                />
              );
            })}
          </Carousel>

          {response.mapping[data[0]?.url]?.children?.length ? (
            <ChildrenHierarchy
              data={response.mapping[data[0]?.url]?.children}
              parentTitle={response.mapping[data[0]?.url]?.title}
              parentUrl={data[0]?.url}
            />
          ) : null}
        </>
      ) : (
